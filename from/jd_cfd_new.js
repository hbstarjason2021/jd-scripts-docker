...
!(async () => {
  ...
  if (!res) {
    $.http.get({url: 'https://gitee.com/starjason/sharecode/raw/master/cfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/cfd.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/starjason/sharecode/raw/master/cfd.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
