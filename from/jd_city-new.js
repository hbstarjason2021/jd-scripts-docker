...
!(async () => {
  ...
  inviteCodes = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/city.json')
  if (!inviteCodes) {
    $.http.get({url: 'https://gitee.com/starjason/sharecode/raw/master/city.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    inviteCodes = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/city.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/starjason/sharecode/raw/master/city.json`, 'timeout': 15000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
