...
!(async () => {
  ...
  $.authorTuanList = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/syj.json');
  if (!$.authorTuanList) {
    $.http.get({url: 'https://gitee.com/starjason/sharecode/raw/master/syj.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorTuanList = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/syj.json') || [];
  }
  const temp = await getAuthorShareCode('https://gitee.com/starjason/sharecode/raw/master/syj.json') || []
