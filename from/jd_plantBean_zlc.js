...
let cookiesArr = [], cookie = '', fullist = [], jdPlantBeanShareArr = [], notify, newShareCodes, message,subTitle;
//京东接口地址
...
function readShareCode(code) {
  ...
  return new Promise(async resolve => {
    $.get({ url: `https://jihulab.com/hbstarjason/sharecode/-/raw/master/plantBean.json`, timeout: 10000 }, (err, resp, data) => { //JD_ZLC_URL + `/bean?code=` + code
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
