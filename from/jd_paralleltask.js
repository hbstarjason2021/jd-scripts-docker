...
!(async() => {
  ...
  for (let i = 8; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
...
function join(venderId, channel, shopId) {
  ...
  let h5st = geth5st([{"key":"appid","value":"jd_shop_member"},{"key":"body","value":$.CryptoJS.SHA256($.toStr(body,body)).toString()},{"key":"client","value":"H5"},{"key":"clientVersion","value":"9.2.0"},{"key":"functionId","value":"bindWithVender"},{"key": "jsonp","value": `jsonp_${time}_${s}`}],process.env.appId)
  return new Promise((resolve) => {
    $.get({
