/*
自动获取宠旺旺的invokeKey
参考：https://github.com/airacg/jd_task/blob/main/jd-task-invokeKey.js
cron 5 0-23/6 * * * jd_get_invokeKey.js
 */
const $ = new Env('宠旺旺获取invokeKey')
const notify = $.isNode() ? require('./sendNotify') : '';
const fs = require('fs');
!(async () => {
  await getInfo()
  if ($.invokeKey) await writeFile();
})()
    .catch((e) => {
      $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
    })
...
function getInfo(url = 'https://prodev.m.jd.com/mall/active/2tZssTgnQsiUqhmg5ooLSHY9XSeN/index.html', type = 1) {
  return new Promise(resolve => {
    $.get({
      ...
      try {
        if (type === 1) {
          data = data && data.match(/"htmlSourceUrl":"([^\"]+)"/);
          if (data && data[1]) {
  ...
  try {
    ...
    if (invokeKey) {
      if (invokeKey !== $.invokeKey) {
        ...
        await notify.sendNotify(`宠旺旺-invokeKey已发生变化`, `新的invokeKey：${$.invokeKey} 已自动写入到utils/config.js，如已是最新 invokeKey 请忽略！`);
