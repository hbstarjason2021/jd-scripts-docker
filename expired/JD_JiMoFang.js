/*
  ...
  修改手动进入活动的判断条件
  修改部分任务的返回逻辑
  根据目前日志观察，收取魔方动作在碎片数小于等于3时出现，根据此修改收取魔方的等待逻辑
  20211230 V1.3
  识别活动界面改为等待1分钟，超时自动退出，避免死循环
  缩短收取魔方等待时间
  缩短任务间等待时间
  20220216 V1.4
  最后汇总显示所有账号执行情况，方便查看
  20220301 V1.5
  缩短部分任务等待时间（可能出现任务卡死的情况，建议备份旧版再运行此版本）
  增加兑换京豆功能，目前仅支持随机得3/10京豆一种兑换，默认不开启
  20220302 V1.6
  修改兑换方式，避免兑换失败
  20220302 V1.6.1
  增加兑换入口位置异常判断
  ...
  20220418 V1.8
  修复兑换奖励后可能出现的报错问题
  可能存在不识别任务列表的情况，多次打开可能识别成功，增加尝试重启任务列表命令
  20220418 V1.8.1
  再次修复兑换奖励后可能出现的报错问题
  20220419 V1.9
  任务中的返回也增加关键字检测，避免由于未正确进入任务又返回，导致退出活动界面的问题
  减少重启任务列表次数
  20220419 V1.9.1
  未识别到任务列表时改用坐标点击方式重启任务列表
  20220425 V1.9.2
  增加任务死循环检测
  20220430 V2.0
  增加逛逛试用会场任务
